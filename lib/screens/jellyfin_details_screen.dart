...
class _JellyfinDetailsScreenState extends State<JellyfinDetailsScreen> {
  ...
  Future<void> _loadDetails() async {
    ...
    try {
      if (widget.item.type == 'Series') {
        ...
        final firstGenre = widget.item.genres.isNotEmpty
            ? widget.item.genres.first
            : null;
        final seriesData = await _jf.loadSeriesData(
          widget.item.id,
        ...
        if (seriesData.seasons.isNotEmpty) {
          ...
          _episodes = _allEpisodes
              .where(
                (ep) =>
                    ep.seasonId == selectedSeason.id ||
                    (ep.parentIndexNumber ?? 1) ==
                        (selectedSeason.indexNumber ?? 1),
              )
              .toList();
          ...
          try {
            ...
            if (canonicalId != null) {
              ...
              final canonData = await _jf.loadSeriesData(
                canonicalId,
                firstGenre: firstGenre,
              );
              ...
              if (_seasons.isNotEmpty) {
                final firstIdx = _seasons.indexWhere(
                  (s) => (s.indexNumber ?? 0) > 0,
                );
                _selectedSeasonIndex = firstIdx >= 0 ? firstIdx : 0;
                final selectedSeason = _seasons[_selectedSeasonIndex];
          ...
          if (sorted.length == 1) {
            ...
            _episodes = _allEpisodes
                .where(
                  (ep) =>
                      (ep.parentIndexNumber ?? 1) ==
                      (_seasons[_selectedSeasonIndex].indexNumber ?? 1),
                )
                .toList();
          }
        }
        ...
        final detailGenre = _details!.genres.isNotEmpty
            ? _details!.genres.first
            : null;
        if (detailGenre != null) {
          try {
  ...
  Future<void> _loadEpisodes(int seasonIndex) async {
    ...
    try {
      if (_usingVirtualSeasons) {
        ...
        _episodes = _allEpisodes
            .where((ep) => (ep.parentIndexNumber ?? 1) == targetSeasonNum)
            .toList();
      } else {
  ...
  void _playItem(JellyfinItem item) async {
    ...
    if (item.type == 'Episode') {
      title =
          '${item.seriesName ?? widget.item.name} - S${item.parentIndexNumber ?? '?'}E${item.indexNumber ?? '?'} - ${item.name}';
    }
  ...
  void _playResume(JellyfinItem item) async {
    ...
    final startPos = result.isTranscode
        ? null
        : Duration(microseconds: posTicks ~/ 10);
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => PlayerScreen(
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          ...
          if (item.type == 'Movie')
            SliverToBoxAdapter(child: _buildPlayButtons(item)),
          if (item.type == 'Series') ...[
                  ...
                  .map(
                    (g) => Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: _jfBlue.withValues(alpha: 0.15),
                        ),
                      ),
                      child: Text(
                        g,
            ...
            child: GestureDetector(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: _jfBlue.withValues(alpha: 0.25),
                      blurRadius: 16,
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      hasProgress ? 'Play from Start' : 'Play',
                      style: const TextStyle(
                        color: Colors.white,
                        fontSize: 16,
                        fontWeight: FontWeight.w700,
              ...
              child: GestureDetector(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(
                      color: _jfBlue.withValues(alpha: 0.5),
                      width: 1.5,
                    ),
                  ),
                  child: Row(
                ...
                child: _HoverCard(
                  onTap: () {
                    Navigator.pushReplacement(
                      context,
                      MaterialPageRoute(
                        builder: (_) => JellyfinDetailsScreen(item: sim),
                      ),
                    );
                  },
                  child: SizedBox(
