...
class _MusicScreenState extends State<MusicScreen>
  ...
  Future<void> _loadTrendingTracks() async {
    ...
    final tracks = await _musicService.getTrendingTracks(
      index: _currentMusicOffset,
      limit: _musicLimit,
    );
    if (!mounted) return;
    setState(() {
      ...
      children: [
        ...
        Positioned(
          ...
          child: Column(
            ...
            children: [
              _buildMiniPlayer(),
              if (_currentView == MusicView.main) _buildPagination(),
            ],
          ),
        ),
      ],
              ...
              actions: [
                if (liked.isNotEmpty)
                  _buildActionButton(Icons.shuffle_rounded, 'Shuffle', () {
                    ...
                    _playerService.playTrack(
                      liked[0],
                      newPlaylist: List.from(liked),
                    );
                  }),
              ],
            ),
              ...
              child: liked.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) =>
                          _buildTrackTile(liked[index], liked, index: index),
                    ),
            ),
          ],
        );
              ...
              actions: [
                if (downloaded.isNotEmpty) ...[
                  _buildActionButton(Icons.shuffle_rounded, 'Shuffle', () {
                    ...
                    _playerService.playTrack(
                      downloaded[0],
                      newPlaylist: List.from(downloaded),
                    );
                  }),
                  const SizedBox(width: 8),
                  _buildActionButton(
                    ...
                    'Folder',
                    () => _openDownloadFolder(downloaded),
                  ),
                ],
              ],
            ),
            if (downloaded.isNotEmpty)
              Padding(
                ...
                child: TextField(
                  ...
                  onChanged: (val) =>
                      setState(() => _downloadedSearchQuery = val),
                  decoration: InputDecoration(
  ...
  Widget _buildPlaylistTile(MusicPlaylist playlist) {
    final coverUrl = playlist.tracks.isNotEmpty
        ...
        Expanded(
          child: _userAlbums.isEmpty
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) => _buildAlbumCard(
                    _userAlbums[index],
                    () => _openAlbum(_userAlbums[index]),
                  ),
                ),
        ),
      ],
          ...
          child: _buildDetailHero(
            ...
            onShuffle: p.tracks.isNotEmpty
                ? () {
                    ...
                    _playerService.playTrack(
                      p.tracks[0],
                      newPlaylist: List.from(p.tracks),
                    );
                  }
                : null,
          ...
          child: _buildDetailHero(
            ...
            onShuffle: _selectedAlbumTracks.isNotEmpty
                ? () {
                    _playerService.isShuffleEnabled.value = true;
                    _playerService.playTrack(
                      _selectedAlbumTracks[0],
                      newPlaylist: List.from(_selectedAlbumTracks),
                    );
                  }
                : null,
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () =>
                          _showTrackMenu(track, fromPlaylist: fromPlaylist),
                        ...
                        onTap: () async {
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  isSaved ? 'Album removed' : 'Album saved!',
                                ),
                                backgroundColor: AppTheme.current.primaryColor
                                    .withValues(alpha: 0.9),
                                behavior: SnackBarBehavior.floating,
  ...
  Widget _buildEmptyState(IconData icon, String title, String subtitle) {
    return DizzyEmptyState(icon: icon, title: title, description: subtitle);
  }
            ...
            if (fromPlaylist != null)
              _buildMenuItem(
                ...
                () async {
                  ...
                  final updatedTracks = List<MusicTrack>.from(
                    fromPlaylist.tracks,
              ...
              builder: (context, downloaded, _) {
                ...
                return _buildMenuItem(
                  ...
                  () async {
                    Navigator.pop(context);
                    if (isDownloaded) {
                      final downloadedTrack = downloaded.firstWhere(
                        (s) => s.id == track.id,
                      );
  ...
  void _openDownloadFolder(List<MusicTrack> downloaded) async {
    ...
    final path =
        downloaded.first.localPath ??
        'Android/media/com.sirius6907.dizzy/Music';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    height: 40,
                    decoration: BoxDecoration(
                      color: AppTheme.current.accentColor.withValues(
                        alpha: 0.15,
                      ),
                      borderRadius: BorderRadius.circular(12),
                    ),
              ...
              const SizedBox(height: 20),
              Text(
                'Android restricts apps from opening folders directly.',
                style: TextStyle(
                  color: Colors.white.withValues(alpha: 0.5),
                  fontSize: 14,
                ),
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Songs are added to your system music library!',
                      style: TextStyle(
                        color: Colors.greenAccent.withValues(alpha: 0.7),
                        fontSize: 12,
                      ),
              ...
              Row(
                ...
                children: [
                  TextButton(
                    ...
                    child: Text(
                      'Copy Path',
                      style: TextStyle(color: AppTheme.current.primaryColor),
                    ),
                  ),
                  const SizedBox(width: 8),
                  TextButton(
